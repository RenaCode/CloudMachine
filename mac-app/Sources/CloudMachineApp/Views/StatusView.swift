                            ...
                            if controller.status.quota.limitGB > 0 {
                                ...
                                if let checked = controller.status.quota.lastChecked {
                                    HStack {
                                        ...
                                        Text("Ostatnia synchronizacja: \(checked.formatted(date: .abbreviated, time: .shortened))")
                                    }
                                    .font(.caption2)
                ...
                VStack(spacing: 12) {
                    HStack(spacing: 12) {
                        ...
                        Button(action: {
                            ...
                            let label = controller.status.mountState == .mounted ? "Odmontuj dysk" : "Zamontuj dysk"
        ...
        HStack(alignment: .top, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text("Wymagany Pełny dostęp do dysku (Full Disk Access)")
                    .font(.headline)
                    .foregroundStyle(.primary)
                Text("System macOS wymaga tego uprawnienia, aby aplikacja mogła sterować kopiami zapasowymi Time Machine. Kliknij poniższy przycisk, przejdź do Ustawień systemowych i dodaj aplikację CloudMachine oraz Terminal do listy.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
