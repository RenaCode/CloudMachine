  ...
  var body: some View {
    ScrollView {
      VStack(alignment: .leading, spacing: 20) {
        ...
        let isRegistered = controller.config.machines.contains(where: {
          $0.key == controller.status.currentMachineKey
        })
        ...
        WizardStepCard(
          ...
          description:
            "Zdefiniuj przyjazną nazwę wyświetlaną dla tego komputera oraz maksymalny limit przestrzeni na kopie zapasowe Google Drive."
        ) {
          VStack(alignment: .leading, spacing: 12) {
            ...
            Button(action: {
              ...
              Task {
                await controller.ensureMachineRegistered(displayName: displayName, limitGB: limit)
              }
            }) {
        ...
        let step4Done =
          controller.status.mountState == .mounted
          && controller.status.timeMachineState == .registered
        WizardStepCard(
          number: 4,
          title: "Zamontuj dysk i zarejestruj w Time Machine",
          status: step4Done ? .completed : (step4Allowed ? .active : .locked),
          description:
            "Zamontuj dedykowany folder z Google Drive jako wirtualny dysk lokalny NFS, a następnie wskaż go jako cel dla Time Machine."
        ) {
          VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 12) {
              Button(action: {
                ...
                Label(
                  isMounted ? "Zamontowano" : "Zamontuj wolumin NFS",
                  systemImage: isMounted ? "folder.fill.badge.checkmark" : "folder.badge.plus")
              }
              .buttonStyle(.bordered)
              ...
              Button(action: {
                ...
                Label(
                  isTM ? "Zarejestrowano" : "Zarejestruj w Time Machine",
            ...
            Text(
              "Rejestracja w Time Machine wymaga jednorazowego wpisania hasła administratora macOS."
            )
            .font(.caption)
            .foregroundStyle(.secondary)
          }
        ...
        let step5Done =
          controller.status.watchdogInstalled && controller.status.mountWatchdogInstalled
        WizardStepCard(
          number: 5,
          title: "Włącz automatyzację w tle (launchd)",
          status: step5Done ? .completed : (step5Allowed ? .active : .locked),
          description:
            "Ostatni krok. Skonfiguruj automatyczne montowanie dysku po starcie systemu, watchdog pilnujący, żeby montowanie nigdy nie zawisło, oraz proces w tle pilnujący limitu pamięci dyskowej."
        ) {
          VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 12) {
              Button(action: {
            ...
            Text(
              "Druga opcja dodaje ograniczoną regułę do sudoers w celu automatycznego usuwania najstarszych kopii zapasowych bez pytania o hasło, gdy skończy się przydzielona przestrzeń."
            )
            .font(.caption)
            .foregroundStyle(.secondary)
          }
