                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        if let info = controller.status.driveInfo {
                            HStack {
                                VStack(alignment: .leading, spacing: 4) {
                                    Text("Rozmiar konta: \(Int(info.totalGB)) GB")
                                        .font(.headline)
                                    Text(String(format: "Wolne: %.0f GB  •  Zajęte: %.0f GB", info.freeGB, info.usedGB))
                                        .font(.subheadline)
                            ...
                            if let checked = info.lastChecked {
                                Text("Ostatnio sprawdzone: \(checked.formatted(date: .abbreviated, time: .shortened))")
                                    .font(.caption2)
                                    .foregroundStyle(.secondary)
                            }
                        } else {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text("Margines zapobiega zapełnieniu całego Google Drive, zostawiając bufor na pliki spoza backupu.")
                                .font(.caption2)
                                .foregroundStyle(.secondary)
                        }
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            if controller.config.isOverBudget {
                                HStack(spacing: 4) {
                                    ...
                                    Text("Suma limitów (\(controller.config.allocatedGB) GB) przekracza bezpieczny budżet (\(controller.config.safeBudgetGB) GB)!")
                                        .font(.caption)
                                        .foregroundStyle(.orange)
                                }
                            } else {
                                Text("Przydzielono \(controller.config.allocatedGB) GB z \(controller.config.safeBudgetGB) GB bezpiecznego budżetu.")
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
                            }
                        }
                ...
                if controller.status.currentMachineKey.isEmpty {
                    ...
                    StatusCard(title: "Ustawienia tej maszyny", systemImage: "laptopcomputer") {
                        VStack(alignment: .leading, spacing: 12) {
                            Text("Bieżący komputer Mac (\(controller.status.currentMachineKey)) nie jest jeszcze skonfigurowany w tej instancji CloudMachine.")
                                .font(.callout)
                                .foregroundStyle(.secondary)
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 12) {
                    ForEach(machines) { machine in
                        LegendItem(color: colorForMachine(machine.key), label: "\(machine.displayName) (\(machine.limitGB) GB)")
                    }
                    ...
                    let remainingGB = max(totalGB - allocatedSum, 0.0)
                    if remainingGB > 0 {
                        LegendItem(color: Color.gray.opacity(0.2), label: "Wolny budżet (\(Int(remainingGB)) GB)")
                    }
                }
                .font(.caption2)
            }
