...
struct CloudMachineAgent: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        abstract: "CloudMachine - montowanie, watchdogi i instalacja. Wolane przez launchd na harmonogramie, albo recznie z Terminala.",
        subcommands: [
            Mount.self,
            Unmount.self,
            MountWatchdog.self,
...
enum CLIContext {
    static func load() async -> (config: MachinesConfig, machineKey: String) {
        ...
        if let corruption {
            CMLogger.log("BLAD: plik konfiguracyjny jest uszkodzony (\(corruption.localizedDescription)) - uzywam pustej konfiguracji w pamieci, oryginal zachowany na dysku z kopia zapasowa obok.")
        }
