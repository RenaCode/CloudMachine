...
public enum RuntimeState {
    public static func setMountDesired(_ desired: Bool) {
        try? (desired ? "on" : "off").write(to: CMPaths.mountDesiredStatePath, atomically: true, encoding: .utf8)
    }
    ...
    public static func startCaffeinate() {
        ...
        try? "\(process.processIdentifier)".write(to: CMPaths.caffeinatePidFile, atomically: true, encoding: .utf8)
    }

    public static func stopCaffeinate() {
        guard let pidString = try? String(contentsOf: CMPaths.caffeinatePidFile, encoding: .utf8),
              let pid = pid_t(pidString.trimmingCharacters(in: .whitespacesAndNewlines)) else { return }
        kill(pid, SIGTERM)
