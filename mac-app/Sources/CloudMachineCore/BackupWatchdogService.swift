...
public enum BackupWatchdogService {
    ...
    private static var stateFile: URL { CMPaths.logDir.appendingPathComponent(".backup-watchdog-last-attempt") }
    ...
    private static func runLocked(config: MachinesConfig, machineKey: String) async {
        ...
        guard await MountHealth.isMounted(localDir.path), await MountHealth.isMounted(spMount.path) else { return }

        guard let destID = await TimeMachineStatus.destinationID(forMountPointContaining: spMount.path) else { return }
        ...
        CMLogger.log("[backup-watchdog] Time Machine jest bezczynne, mimo ze CloudMachine (\(destID)) powinno byc aktywne - wznawiam.")
        ...
        if result?.succeeded == true {
            ...
            CMLogger.log("[backup-watchdog] Nie udalo sie wznowic backupu (sudo bez hasla niedostepne lub inny blad) - sprawdz regule w /etc/sudoers.d/cloudmachine.")
        }
    }
}
