  ...
  public static func registerUnattended(machineKey: String) async -> CMActionResult {
    ...
    guard await MountHealth.isMounted(spMount) else {
      return CMActionResult(
        succeeded: false,
        message:
          "Wirtualny dysk sparsebundle pod \(spMount) nie jest zamontowany. Uruchom najpierw montowanie."
      )
    }
    ...
    guard result?.succeeded == true else {
      return CMActionResult(
        succeeded: false,
        message:
          "Rejestracja celu Time Machine nie powiodla sie (sudo bez hasla niedostepne lub inny blad)."
      )
    }
    return CMActionResult(
      succeeded: true, message: "Zarejestrowano \(spMount) jako cel Time Machine.")
  }
}
