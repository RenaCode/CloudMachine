...
public enum QuotaWatchdogService {
    ...
    private static func runLocked(config: MachinesConfig, machineKey: String) async {
        ...
        guard isLimitSane(limitGB) else {
            CMLogger.log("BLAD: limit skonfigurowany dla '\(machineKey)' (\(limitGB) GB) jest podejrzanie niski (<\(minSaneLimitGB) GB) - to prawie na pewno pomylka w konfiguracji, nie zamierzona wartosc. Przerywam, ZEBY NIE SKASOWAC wszystkich backupow. Popraw limit recznie.")
            return
        }

        let triggerGB = Self.triggerGB(forLimitGB: limitGB)
        CMLogger.log("Sprawdzam wykorzystanie \(remotePath) (limit \(limitGB) GB, prog przycinania \(triggerGB) GB)")
        ...
        guard await MountHealth.isMounted(spMount.path) else {
            CMLogger.log("BLAD: przekroczono prog, ale wirtualny dysk sparsebundle pod \(spMount.path) nie jest zamontowany - nie moge wylistowac backupow TM do przyciecia.")
            return
        }

        CMLogger.log("Przekroczono prog przycinania. Szukam najstarszych backupow Time Machine do usuniecia.")

        guard let listResult = try? await ProcessRunner.run("/usr/bin/tmutil", ["listbackups", "-d", spMount.path]) else {
            return
        }
        ...
        guard backups.count > 1 else {
            CMLogger.log("UWAGA: zostal juz tylko \(backups.count) backup(ow) - nie usuwam, zeby nie zostac bez kopii zapasowej. Rozwaz podniesienie limitu.")
            return
        }
        ...
        for backupPath in backups {
            ...
            let deleteResult = try? await ProcessRunner.run("/usr/bin/sudo", ["-n", "/usr/bin/tmutil", "delete", "-p", backupPath])
            ...
            guard let refreshed = await RcloneSize.usedGB(remotePath: remotePath, timeout: 180) else { break }
            usedGB = refreshed
        ...
        if deleted > 0 {
            _ = try? await ProcessRunner.run("/usr/bin/osascript", [
                "-e", "display notification \"Usunieto \(deleted) najstarszych backupow, zeby zmiescic sie w limicie \(limitGB) GB\" with title \"CloudMachine\""
            ])
        }
    }
}
