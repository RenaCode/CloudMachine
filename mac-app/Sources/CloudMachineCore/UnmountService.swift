...
public enum UnmountService {
    public static func unmount(config: MachinesConfig, machineKey: String) async {
        ...
        if await TimeMachineStatus.isRunning() {
            CMLogger.log("Time Machine aktywnie kopiuje - zatrzymuje backup przed odmontowaniem, zeby nie uszkodzic sparsebundle.")
            ...
            if await TimeMachineStatus.isRunning() {
                CMLogger.log("OSTRZEZENIE: Time Machine nie zatrzymalo sie po \(waited)s - odmontowuje mimo to, ryzyko uszkodzenia sparsebundle.")
            } else {
        ...
        if unmount?.succeeded != true {
            let diskutilUnmount = try? await ProcessRunner.run("/usr/sbin/diskutil", ["unmount", localDir.path])
            if diskutilUnmount?.succeeded != true {
                ...
                if !(await MountHealth.forceUnmount(localDir.path, timeoutS: 10)) {
                    CMLogger.log("OSTRZEZENIE: punkt montowania nadal widnieje w tabeli po probie wymuszonego odmontowania - proces diskutil moze zostac osierocony w tle.")
                }
            }
        }
        await MountHealth.killRcloneForRemote(remotePath)
