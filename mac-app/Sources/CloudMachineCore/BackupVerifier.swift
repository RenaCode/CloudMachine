...
public enum BackupVerifier {
    ...
    public static func latestBackupPath(spMount: String) async -> String? {
        guard let result = try? await ProcessRunner.run("/usr/bin/tmutil", ["listbackups", "-d", spMount]) else {
            return nil
        }
    ...
    public static func runFullCheck(config: MachinesConfig, machineKey: String) async -> CMActionResult {
        ...
        guard await MountHealth.isMounted(spMount) else {
            return CMActionResult(succeeded: false, message: "Wirtualny dysk sparsebundle pod \(spMount) nie jest zamontowany.")
        }
        ...
        guard let latestBackup = await latestBackupPath(spMount: spMount) else {
            return CMActionResult(succeeded: false, message: "Brak backupu do zweryfikowania. Odpal najpierw: sudo tmutil startbackup --auto --block")
        }

        CMLogger.log("=== 3/3: weryfikacja sum kontrolnych najnowszego backupu: \(latestBackup) (moze potrwac dlugo) ===")
        guard let verifyResult = try? await ProcessRunner.runTmutilUnattended(["verifychecksums", latestBackup]), verifyResult.succeeded else {
            return CMActionResult(succeeded: false, message: "verifychecksums zglosilo problem - montowanie przez rclone nfsmount moze nie byc wystarczajaco niezawodne dla Time Machine.")
        }
        ...
        guard await rcloneCheck(localDir: localDir, remotePath: remotePath) else {
            return CMActionResult(succeeded: false, message: "rclone check wykazalo roznice miedzy lokalnym widokiem a stanem na Google Drive.")
        }

        return CMActionResult(succeeded: true, message: "Wszystkie testy przeszly. Backup wyglada na spojny.")
    }
}
