...
public enum MountWatchdogService {
    ...
    /// cyklach pod rzad.
    public static func decide(nfsListed: Bool, isResponsive: Bool, isBusyDraining: Bool, currentStreak: Int) -> (decision: MountWatchdogDecision, newStreak: Int) {
        if nfsListed {
            if isResponsive {
                return (.healthy, 0)
            }
    ...
    private static var streakFile: URL { CMPaths.logDir.appendingPathComponent(".mount-watchdog-slow-streak") }
    ...
    public static func run(config: MachinesConfig, machineKey: String) async {
        guard await withCMLock("mount-watchdog", { await runLocked(config: config, machineKey: machineKey) }) != nil else {
            return
        }
    }
    ...
    private static func runLocked(config: MachinesConfig, machineKey: String) async {
        ...
        let isResponsive = nfsListed ? await MountHealth.probeResponsive(localDir.path, timeoutS: 20) : false
        let isBusyDraining = nfsListed ? false : await MountHealth.rcloneIsBusyDraining(remotePath, quietThreshold: 45)
        let currentStreak = readStreak()

        let (decision, _) = Self.decide(
            nfsListed: nfsListed, isResponsive: isResponsive, isBusyDraining: isBusyDraining, currentStreak: currentStreak
        )
        ...
        case .healthy:
            ...
            if !(await MountHealth.isMounted(spMount.path)) {
                CMLogger.log("[mount-watchdog] NFS zdrowy, ale wirtualny dysk sparsebundle nie jest zamontowany pod \(spMount.path) - montuje.")
                ...
                let result = try? await ProcessRunner.run("/usr/bin/hdiutil", [
                    "attach", "-noverify", "-noautoopen", "-nobrowse", "-mountpoint", spMount.path, sparsebundlePath
                ])
                if result?.succeeded != true {
                    CMLogger.log("[mount-watchdog] Nie udalo sie zamontowac sparsebundle w tym przebiegu, sprobuje ponownie w nastepnym cyklu.")
                }
            }
            return
        ...
        case .waitingForConfirmation(let streak):
            writeStreak(streak)
            CMLogger.log("[mount-watchdog] \(localDir.path) nie odpowiedzialo w 20s (proba \(streak)/\(slowStreakThreshold)) - czekam na potwierdzenie w kolejnym cyklu przed naprawa.")
            return

        case .waitingForRcloneDraining:
            CMLogger.log("[mount-watchdog] \(localDir.path) jeszcze nie zamontowane, ale rclone wciaz aktywnie pracuje (prawdopodobnie doganiam zalegle wysylki po przerwanym backupie) - czekam, nie przerywam.")
            return

        case .needsRepair:
            clearStreak()
            if nfsListed {
                CMLogger.log("[mount-watchdog] \(localDir.path) nie odpowiada od \(slowStreakThreshold) kolejnych sprawdzen - traktuje jako zawieszone.")
            } else {
        ...
        if nfsListed {
            ...
            if await MountHealth.isMounted(localDir.path) {
                CMLogger.log("[mount-watchdog] Punkt montowania nadal widnieje w tabeli po probie wymuszonego odmontowania - mount przeniesie go na bok i utworzy nowy.")
            }
        }
        ...
        if success {
            ...
            _ = try? await ProcessRunner.run("/usr/bin/osascript", [
                "-e", "display notification \"Wykryto zawieszone montowanie dysku Google i naprawiono je automatycznie.\" with title \"CloudMachine\""
            ])
        } else {
            CMLogger.log("[mount-watchdog] Naprawa nie powiodla sie w tym przebiegu, sprobuje ponownie przy nastepnym cyklu watchdoga.")
        }
    }
}
