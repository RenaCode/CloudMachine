...
public enum DependencyInstaller {
    ...
    public static func installRclone() async -> CMActionResult {
        guard let brewPath = resolvedBrewPath() else {
            return CMActionResult(succeeded: false, message: "Homebrew nie jest zainstalowany. Zainstaluj go recznie: https://brew.sh")
        }
        let result = try? await ProcessRunner.run(brewPath, ["install", "rclone"], timeout: 600)
        guard result?.succeeded == true else {
            return CMActionResult(succeeded: false, message: "Instalacja rclone nie powiodla sie: \(result?.stderr ?? "nieznany blad")")
        }
