...
final class MountWatchdogServiceTests: XCTestCase {
  func testDecide_mountedAndResponsive_isHealthy() {
    let (decision, streak) = MountWatchdogService.decide(
      nfsListed: true, isResponsive: true, isBusyDraining: false, currentStreak: 2)
  ...
  func testDecide_mountedButUnresponsive_belowThreshold_waitsForConfirmation() {
    let (decision, streak) = MountWatchdogService.decide(
      nfsListed: true, isResponsive: false, isBusyDraining: false, currentStreak: 0)
  ...
  func testDecide_mountedButUnresponsive_reachesThreshold_needsRepair() {
    ...
    let (decision, streak) = MountWatchdogService.decide(
      nfsListed: true, isResponsive: false, isBusyDraining: false, currentStreak: 2)
  ...
  func testDecide_notMountedButRcloneBusyDraining_waits() {
    ...
    let (decision, streak) = MountWatchdogService.decide(
      nfsListed: false, isResponsive: false, isBusyDraining: true, currentStreak: 0)
  ...
  func testDecide_notMountedAndRcloneIdle_needsRepairImmediately() {
    ...
    let (decision, streak) = MountWatchdogService.decide(
      nfsListed: false, isResponsive: false, isBusyDraining: false, currentStreak: 0)
    XCTAssertEqual(decision, .needsRepair)
    XCTAssertEqual(streak, 0)
  }
}
